...
void Transform::RotateAround(const XMFLOAT3& point, const XMFLOAT3& axis, float radian)
{
    ...
    // 以point作为原点进行旋转
    XMMATRIX RT = XMMatrixRotationRollPitchYawFromVector(rotationVec) * 
                  XMMatrixTranslationFromVector(positionVec - centerVec);
...
{
    XMMATRIX View = XMMatrixLookAtLH(XMLoadFloat3(&m_Position), 
                                     XMLoadFloat3(&target), 
                                     XMLoadFloat3(&up));
...
{
    XMMATRIX View = XMMatrixLookToLH(XMLoadFloat3(&m_Position), 
